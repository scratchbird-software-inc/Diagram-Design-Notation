 ...
 */
  ...
  function parse$1(text, source='input.ddn') {
  ...
  function createWorkspace$1(files,entry){
    ...
    function load(path){
      ...
      const open=p=>{
        ...
        active.add(p);const d=parse$1(files[p],p);docs.set(p,d);d.imported=new Map();
  ...
  function build(files,entry,viewName,registry,stack=[]){
    ...
    const ws=createWorkspace$1(files,entry);const all=[...ws.symbols.values()];const view=all.find(n=>n.type==='view'&&((viewName&&(n.id===viewName||n.path===viewName||n.uid===viewName))||(!viewName&&n.doc===ws.main)));
  ...
  const api$e={VERSION: VERSION$4,SOURCE_VERSIONS,DDNError,lex,parse: parse$1,bundle,createWorkspace: createWorkspace$1,build,children,group,values,getFields,fieldTree,getPorts,clean,quantity,kindEntry,relationEntry,semanticJSON,DEFAULTS,PROPERTIES,CHOICES,profiles:api$g};
