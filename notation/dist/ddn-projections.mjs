...
if (!host.DDNProjections) {
  host.DDNProjections = api$1;
  const Engine = optionalNamespace('DDNEngine');
  for (const k of ['chart', 'matrix', 'panels', 'timeline', 'table', 'sequence', 'timing', 'chen']) Engine.registerProjectionRenderer(k, api$1.render);
}
...
export { api$1 as DDNProjections, VERSION, authoring, createWorkspace, api as default, fromSnapshot, io, mount, parse, profileCatalogue, registerWorkspace, runtime };
