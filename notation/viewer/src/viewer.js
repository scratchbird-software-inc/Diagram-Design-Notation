 ...
 * Deprecated (B1-027): superseded by the unified tool (notation/tool/).
 * Sources kept for the test suite; no longer served by the website.
...
/* B1-026 (D1): import closure for ?src= deep links. Fetches the entry file and
 * recursively every file it imports, keyed by workspace-relative name, so
 * multi-file examples render instead of dying on DDN022. `fetchFn(url)` must
 * return a fetch-like Response; `parseImports(text, name)` returns import path
 * strings; `resolvePath(name, path)` gives the workspace-relative file name
 * (DDNLive.parse / DDNLive.resolvePath in the browser, stubs in node tests).
 * Cycle-safe: a file is recorded before its imports are followed, so re-visits
 * stop at the hasOwnProperty guard. Fetch failures name the missing file. */
function srcImportClosure(src, pageHref, fetchFn, parseImports, resolvePath) {
  ...
  const pull = (url, name) => fetchFn(url).then(r => {
    ...
    if (text.length > MAX_FILE_BYTES)
      throw new Error(name + ' is ' + Math.round(text.length / 1e6) + ' MB — the viewer accepts sources up to ' + (MAX_FILE_BYTES / 1e6) + ' MB');
...
/* B1-023 (D1) + B1-026 (D1): deep link — `index.html?src=<relative .ddn path>`
 * fetches the source next to the page together with its whole import closure
 * (multi-file examples) and loads it as a workspace, first view selected. Every
 * fetched text is size-capped like a dropped file and parsed via the normal
 * loadFiles path, so parse errors show in the standard status line. */
