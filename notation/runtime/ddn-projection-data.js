...
function plan(ir,ErrorClass=Error){
 ...
 if(kind==='panels'){
  ...
  const CANVAS={'canvas.bmc@1':[['kp','KEY PARTNERS'],['ka','KEY ACTIVITIES'],['kr','KEY RESOURCES'],['vp','VALUE PROPOSITIONS'],['cr','CUSTOMER RELATIONSHIPS'],['ch','CHANNELS'],['cs','CUSTOMER SEGMENTS'],['cost','COST STRUCTURE'],['rev','REVENUE STREAMS']],'canvas.lean@1':[['problem','PROBLEM'],['solution','SOLUTION'],['keymetrics','KEY METRICS'],['uvp','UNIQUE VALUE PROPOSITION'],['unfair','UNFAIR ADVANTAGE'],['channels','CHANNELS'],['segments','CUSTOMER SEGMENTS'],['cost','COST STRUCTURE'],['revenue','REVENUE STREAMS']]};
  const need=CANVAS[p.profile];if(need)for(const [id,title]of need)if(!p.panels.some(v=>v.id===id))fail('DDN-PJ080',p.profile+' requires panel "'+id+'" ('+title+'); declare all 9 blocks');
