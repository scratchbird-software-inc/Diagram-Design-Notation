...
function main(){
 ...
 if(command==='render'){const library=fs.readFileSync(path.join(__dirname,'../../standard/registry/glyph-library.svg'),'utf8'),defs=library.match(/<defs>([\s\S]*?)<\/defs>/)[1];output=Render.render(ir,registry,defs,{noMotion:flag('--no-motion')}).svg;}
