...
- **DDN 0.7.0** (B1-048): packaging release over the 0.6.0-beta.1 language
  baseline — every version stamp moves to `0.7.0` (runtime `VERSION`
  constants, `@ddn/notation` and root package versions, CLI `--version`,
  specification front matter, registry/validation version fields,
  AI-REFERENCE header, and every version text emitted into rendered output
  and built pages); the designer moves to `0.2.0`. Not a language change:
  source versions stay `0.2`–`0.5`, the core registry stays `ddn-core@0.3`.
  - Corpus conformance: `tools/normalize-ddn.mjs` gained a stable,
    idempotent human-readable whitespace pass (formatting only; never
    verbose↔compact conversion), applied to every `.ddn` in the repo;
    `--check` is clean and running the tool twice is a no-op.
  - Golden audit: every committed golden (use-cases manifest, gallery) was
    regenerated by the 0.7.0 runtime; post-normalize re-renders are
    byte-identical except the version-stamp text.
  - Example-link audit: a permanent headless-Chromium crawl gate (wired
    into root `npm test`) verifies every "Open in" link on the examples
    index, gallery and use-cases pages actually renders in the tool over
    HTTP (`data-ddn-rendered` marker); all root causes found are fixed.
  - Site stamps: every website page stating a version now states `0.7.0`,
    sourced from the single `notation/package.json` version via
    `website/build-site.mjs`; stale `0.5`/`0.6` stamps swept.
  - CLI: `node notation/cli/cli.js --version` prints the runtime version.
