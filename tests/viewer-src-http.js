 ...
 * The URLs pin ?worker=off: --dump-dom --virtual-time-budget does not wait for
 * worker messages, so the default worker render path (covered by
 * tests/tool-worker-http.js) would race the DOM dump.
 */
'use strict';
...
try {
  ...
  test('retired viewer URL redirects to the tool preserving ?src=', () => {
    ...
    const dom = dumpDom('http://127.0.0.1:' + PORT + '/website/tools/viewer/index.html?worker=off&src=../../examples/basics/61-self-contained.ddn');
  ...
  test('retired studio editor URL redirects to the tool preserving entry/view', () => {
    const dom = dumpDom('http://127.0.0.1:' + PORT + '/website/tools/studio/editor.html?worker=off&entry=examples%2Flive%2Fadaptive-lab.ddn&view=automatic');
