...
const BUNDLES = {
  ...
  iso: { blurb: 'Optional isometric module: axonometric 30° projection, face shading, chart extrusions (bar/pie/donut/area/treemap) and iso graph prisms (optional: visible placeholder when absent).' },
  global: { blurb: 'unified public runtime (no old compatibility renderer)' },
};

const SIBLING_SPEC = /^\.\/ddn-(core|graph|quality|projections|geo|iso)\.js$/;
...
function pathsFor() {
  return id => {
    ...
    const base = path.basename(id);
    if (/^ddn-(core|graph|quality|projections|geo|iso)\.js$/.test(base)) return './' + base;
    const owner = memberFile.get(base);
...
function configsFor(name) {
  ...
  const intros = {
    ...
    iso: `var h=typeof globalThis!=='undefined'?globalThis:this;if(!h.DDNLive)throw new Error('ddn-iso requires ddn-core.js to be loaded first');if(!h.DDNRender)throw new Error('ddn-iso requires ddn-graph.js to be loaded first');if(h.DDNLive.VERSION!==${V})throw new Error('A different DDNLive runtime is already loaded. Load exactly one version.');if(h.DDNIso)return;`,
