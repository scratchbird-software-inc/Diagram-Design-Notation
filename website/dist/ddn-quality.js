...
if(host.DDNQualityRender)return;
...
for(const k of['fishbone','decision'])host.DDNEngine.registerProjectionRenderer(k,(kind=>(ir,reg,g,opts)=>{if(!host.DDNProjections)throw new host.DDN.DDNError('DDN-E010','Projection kind "'+kind+'" renders through ddn-projections.js; load it together with ddn-quality.js.');return host.DDNProjections.render(ir,reg,g,opts);})(k));
