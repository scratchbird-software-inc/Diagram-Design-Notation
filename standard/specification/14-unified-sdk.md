...
The runtime also ships as optional modular bundles so a page can load only what it displays: `dist/ddn-core.js` (parse/build/validate/export, projection and quality planning data, and the workspace API — no rendering), `dist/ddn-graph.js` (graph renderer; registers the `graph` projection kind), `dist/ddn-projections.js` (chart/matrix/panels/timeline/table/sequence/timing/chen) and `dist/ddn-quality.js` (quality charts, decision tables, fishbone). The optional sixth bundle `dist/ddn-geo.js` (B1-025: mercator/equirectangular/albers/equalEarth projection math, GeoJSON ingestion, choropleth/symbol/outline map rendering behind the `geo.choropleth@1`/`geo.symbols@1`/`geo.outline@1` profiles) is never part of the all-in-one; its `geo` kind registers `optional: true`, so a geo view rendered without it degrades to a visible inline placeholder ("Map view requires ddn-geo.js") plus the coded `DDN-E010` diagnostic on the diagnostics channel rather than throwing. The optional seventh bundle `dist/ddn-iso.js` (B1-034: axonometric 30° isometric depth — chart extrusions for bar/pie/donut/area/treemap and iso graph prisms behind `iso: true`/`depth` on graph and chart projections; spec chapter 43) publishes the `DDNIso` namespace without registering a kind; an `iso: true` view rendered without it degrades to the same style of visible inline placeholder ("Isometric view requires ddn-iso.js") plus the coded `DDN-E010` diagnostic, and a `depth` property without it renders flat with the coded diagnostic. Geography data is a separate optional asset (`assets/geo/world-110m.json`, Natural Earth 110m, ~96 KB) referenced by name/URL or bound inline as GeoJSON — never embedded in any bundle. Load order is core first, then graph, then quality, projections and geo in any order; each non-core bundle throws immediately when its prerequisite is missing, loading the same bundle twice is a no-op, and mixed versions keep the single-version guard. The engine is a renderer registry (`DDNEngine.registerProjectionRenderer(name, fn)`); rendering or planning an unregistered kind throws coded error `DDN-E010` naming the kind and the bundle that provides it. The fishbone and decision renderers compose the page through `ddn-projections.js`, so those kinds need both quality and projections loaded at render time. `ddn.global.js` remains the unchanged all-in-one build (everything plus the Studio component); every `ddn-X.js` has a matching `ddn-X.mjs` facade and type copies, and `dist/README.md` (generated at build time) lists contents, guards and byte sizes. The all-in-one and the modules MUST NOT be mixed on one page.
