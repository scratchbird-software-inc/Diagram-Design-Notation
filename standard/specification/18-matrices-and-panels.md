...
`panels.pyramid@1` binds a pyramid/hierarchy-tier diagram to the same panel compositor under a fixed band contract: `columns:1`, 3–5 panels, each panel a band with `column:0`, `rowspan:1`, `colspan:1`, and `row` equal to its band index — row 0 is the TOP band, row N−1 the bottom, and rows must be contiguous `0..N−1`. Band-count outside 3–5, any non-single-column span, or a gap in the row sequence fails with `DDN-PJ089`, naming the rule that fired. Bands render as trapezoid polygons stacked widest-at-bottom: band `i` has top width `W*(i+1)/(N+1)` and bottom width `W*(i+2)/(N+1)`, so the bottom band spans the full drawing width and the apex stays flat. The band `title` renders centered inside the trapezoid; panel `items` render as optional side annotations to the right of the band edge. Under this profile only, a band may omit `items` (the generic `DDN-PJ009` non-empty-items rule is relaxed for pyramid bands; every other panels profile keeps it). Pyramid bands never carry child views. The pyramid is a tier diagram: band widths come from the fixed width profile, **not** from data values — no value encoding, 3D/pyramidal chart geometry, auto-sizing from data or nested child views.

### Venn diagrams

`panels.venn@1` binds a two- or three-set venn diagram to the same panel compositor under a fixed set contract: exactly 2 or 3 panels, one per set, each panel `id` the set id and `title` the set's display name (the declared grid values — `columns:1`, `column:0`, `rowspan:1`, `colspan:1`, `row` the set index — exist only to satisfy the generic span checks; the renderer ignores the grid). Any other set count fails with `DDN-PJ090`. Each set panel lists its members in `items`; a member belonging to several sets is listed in every one of its set panels (the §18.3 repeated-item rule) and carries the registered `x_sets` extension property — an array of 1–3 unique set ids — declaring exactly the set panels that list it. A missing, malformed, or inconsistent `x_sets` (unknown set id, or disagreement with the panel listings) fails with `DDN-PJ091`, naming the member. Region labels show declared membership counts only (v1 does not print item names inside regions; names remain available via mark provenance), positioned at fixed geometric offsets from the circle centers — single-set regions outward along the center line, pairwise regions at the pushed-out midpoint, the triple region at the centroid — with `0` rendered for empty regions. The circles use fixed equal-radius geometry (two side by side, three in a triangle); **no area-proportional geometry, no 4+ sets**.
