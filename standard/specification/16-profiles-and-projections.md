...
A profile describes a projection, eligible semantics, validation obligations, supported silhouettes, scope and exclusions. The profile library adds 45 kind definitions and 22 relationship definitions to the 152-kind/91-relation core without changing the latter's IDs. Three of the additional kind recipes are used by Chen-generated occurrences. An icon count is not a supported-diagram count.

Registry catalogue note (additive growth): kind entries in both catalogues may carry an optional `defaults` object (property name → default value, `{}` when a kind has no meaningful defaults). This is additive registry growth under the versioning rules — no RFC and no semantics change — and it never extends a published profile entry; defaults live on kind entries only and are type-checked against the property contracts at schema-build time (chapter 04 §10, chapter 10).
...
The graph projection also carries `concept.map@1`: a concept map — concept-family nodes (`object`, `entity`, `term` or `domain` participants) joined by explicitly labelled structural links (`assoc` or `ref`), with no root or cycle constraints (concept maps are not trees). Because the IR merges an unnamed relation with its verb's registry default name, explicitness is checked against that default: validation rejects non-concept participants and non-structural links (`DDN-PF007`), and any selected relation whose effective label is empty or equals the verb default such as `"associated with"` (`DDN-PJ104`) — labels are mandatory and author-written, never inferred. Label inference from endpoints, automatic layout to ontology standards and OWL/RDF export are unsupported.

The graph projection also carries `epc.basic@1`: an Event-driven Process Chain — `epk.event` hexagons and `epk.function` rounded rectangles strictly alternating through `epk.connector` circles carrying an `and`/`or`/`xor` operator, linked only by the `epk.next` verb and rendered left-to-right with the layered layout. Validation rejects non-EPC participants and links (`DDN-PF007`), event-to-event or function-to-function edges (`DDN-PJ105`), connectors without a valid `x_epc.operator` or `x_epc.operator` on a non-connector (`DDN-PJ106`), and attribute compartments on EPC symbols (`DDN-PF003`). Process simulation, BPMN interchange and connector fan-in/fan-out balancing rules are unsupported. This is a DDN profile on the graph projection, not a claim of EPC specification conformance.
