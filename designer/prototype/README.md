...
- Inspect actual source and download DDN, the source workspace ZIP, or the current successful render as SVG, PNG (2×) or WebP (2×). WebP is feature-detected: where the browser cannot encode it the button disables with a visible reason instead of writing a mislabeled file; the renderFailure guard blocks every diagram format.
- Adjust the chrome itself: Compact/Comfortable density toggle in the projectbar (persisted, Compact default); drag the splitters between shelf, canvas and inspector (double-click resets, persisted); detach the shelf or inspector into a floating, draggable window (the canvas grows to fill; re-attach restores the splitter position; floating is session-only); collapse any bottom sheet with its head toggle. Palette buttons render each kind's real notation plate glyph (via the runtime's additive `DDNLive.glyphs.forKind`) and every control has a descriptive tooltip.
