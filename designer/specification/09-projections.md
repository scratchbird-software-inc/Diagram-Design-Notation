...
Implementation status (ED-012): for the RT-101 `uml.sequence@1` projection (`kind:sequence`), lifeline and message editing is implemented in the prototype with declaration-order semantics — participants and messages render in declaration order, and every reorder is a reviewed declaration-span move (`Commands.moveDeclaration`), never vertical pixels as time; two-click connect on lifeline headers and the sequence sheet's message list are equivalent paths (VE-006); a message's dashed return toggles `x_return` true-or-absent. Combined fragments remain unclaimed, and the experimental `session-bootstrap@0.1` interaction-lane mode (`ddn-interaction.js`) remains separate.
